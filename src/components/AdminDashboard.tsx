...
export const AdminDashboard: React.FC = () => {
  ...
  const bgColor = isDark ? '#1e293b' : 'transparent'; // خلفية الصفحة (Slate-800)
  const cardColor = isDark ? '#334155' : '#ffffff'; // خلفية الكروت (Slate-700)
  const borderColor = isDark ? '#475569' : '#e5e7eb'; // إطارات الكروت (Slate-600)
  const textColor = isDark ? '#f8fafc' : '#1f2937'; // النص الأساسي أبيض
  const textMuted = isDark ? '#cbd5e1' : '#6b7280'; // النص الفرعي رمادي فاتح
      ...
      <div className="flex items-center gap-2.5">
        {imageUrl ? (
          <img
            ...
            title={language === "ar" ? "اضغط لتكبير الصورة" : "Click to enlarge image"}
            onClick={(e) => {
              e.stopPropagation();
  ...
  const dynamicCourses = useMemo(() => {
    ...
    records.forEach((r) => {
      if (r.courseId) {
        map.set(
          r.courseId,
          r.courseName || mockCourses.find((c) => c.id === r.courseId)?.title || r.courseId,
        );
      }
    });
  ...
  const handleApprove = (id: string) => {
    ...
    if (approvedUser && approvedUser.fcmToken) {
      sendPushNotification(
        language === "ar" ? "تمت الموافقة على الحساب" : "Account Approved",
        language === "ar" ? "أهلاً بك! تم تفعيل حسابك بنجاح في النظام." : "Welcome! Your account has been activated.",
        [approvedUser.fcmToken]
      );
    }
    alert(language === "ar" ? "تم قبول المتدرب وإرسال إشعار له" : "Approved! (Notification sent to user)");
  };
  ...
  const handleDeleteUser = (id: string) => {
    if (confirm(language === "ar" ? "هل أنت متأكد من حذف هذا المستخدم؟" : "Are you sure you want to delete this trainee?")) {
      setUsers(
    ...
    if (editingSessionId) {
      ...
      const targetAudienceUsers = users.filter((u) => {
        ...
        if (targetParticipants === "engineers") {
          return roleInfo.includes("engineer") || roleInfo.includes("مهندس") || roleInfo.includes("eng");
        }
        if (targetParticipants === "technicians") {
          return roleInfo.includes("technician") || roleInfo.includes("فني") || roleInfo.includes("tech");
        }
        return true;
      });
      ...
      if (validTokens.length > 0) {
        ...
        sendPushNotification(
          ...
          language === "ar"
            ? `تم الإعلان عن دورة جديدة: ${courseTitle} (${startDate}) للفئة المستهدفة: ${targetLabelAr}`
            : `New session announced: ${courseTitle} (${startDate}) for ${targetParticipants}`,
          validTokens
        );
      }
      ...
      addAnnouncement({
        ...
        title: language === "ar" ? `دورة جديدة: ${courseTitle}` : `New Session: ${courseTitle}`,
        message:
          language === "ar"
            ? `تم فتح باب التسجيل لدورة ${courseTitle} (${startDate} إلى ${endDate}) للفئة: ${
                targetParticipants === "engineers"
                  ? "المهندسين"
                  : targetParticipants === "technicians"
                  ? "الفنيين"
            ...
            : `Registration is open for ${courseTitle} from ${startDate} to ${endDate} (${targetParticipants}) at ${location}.`,
        date: new Date().toISOString(),
        author: "Admin / OED",
        isGlobal: targetParticipants === "mixed" || targetParticipants === "all" || !targetParticipants,
        targetAudience: targetParticipants,
      });
  ...
  const handleCancelSession = (sessionId: string) => {
    if (window.confirm(language === "ar" ? "إلغاء هذه الدورة التدريبية؟" : "Cancel this session?")) {
  ...
  const handleSendReminder = (
    ...
    const typeLabel = isFinal ? (language === "ar" ? "تذكير نهائي" : "FINAL REMINDER") : (language === "ar" ? "تذكير عادي" : "Standard reminder");

      const validTokens = users.filter(u => u.fcmToken).map(u => u.fcmToken as string);
      if (validTokens.length > 0) {
        sendPushNotification(
          isFinal ? (language === "ar" ? "تنبيه التسجيل النهائي" : "Final Registration Alert") : (language === "ar" ? "تنبيه تسجيل الدورة" : "Course Registration Alert"),
          language === "ar" ? `تم إرسال تنبيه ${typeLabel} بنجاح لجميع المسجلين في دورة [${session.courseTitle}]!` : `Reminder for course ${session.courseTitle}`,
          validTokens
        );
      }
      
      const toastMsg = language === "ar"
          ? `تم إرسال ${typeLabel} بنجاح لجميع المسجلين في دورة [${session.courseTitle}]!`
  ...
  const getAdminReportOptions = (): ReportOptions => {
    const reportTitle = isSingleTraineeFiltered
      ? language === "ar"
        ? "تقرير المتدرب الشخصي" : "Individual Trainee Training Report"
      : language === "ar"
        ? "تقرير التدريب الشامل" : "Technical Training Report";
    return {
      title: reportTitle,
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        alert(language === "ar" ? "فشل قراءة الملف" : "Failed to parse Excel file.");
      }
    };
  ...
  const kpiStats = useMemo(() => {
    ...
    filteredRecords.forEach((r) => {
      ...
      if (u) {
        ...
        if (roleStr.includes("eng") || roleStr.includes("مهندس")) engineersCount++;
        if (roleStr.includes("tech") || roleStr.includes("فني")) techniciansCount++;
        if (roleStr.includes("op") || roleStr.includes("مشغل")) operatorsCount++;
      }
    });
    return {
  ...
  return (
    <div className="min-h-screen pb-12 transition-colors duration-300" style={{ backgroundColor: bgColor }}>
      <div className="max-w-7xl mx-auto px-4 py-8 print:p-0">
        ...
        {/* Print Button Section - نقل الزرار لأقصى اليمين */}
            ...
            <div className="space-y-12">
              ...
              <div className="md:w-64 shrink-0 flex flex-col space-y-2 border-b md:border-b-0 md:border-r rtl:border-r-0 rtl:border-l pb-4 md:pb-0 md:pr-4 rtl:md:pl-4" style={{ borderColor: borderColor }}>
                <button
                  ...
                  className={`text-left rtl:text-right px-4 py-3 rounded-lg font-medium transition-colors ${userManagementTab === 'pending' ? 'bg-[#002D62] text-white dark:bg-blue-600' : 'hover:bg-gray-100 dark:hover:bg-slate-800'}`}
                ...
                <button
                  ...
                  className={`text-left rtl:text-right px-4 py-3 rounded-lg font-medium transition-colors ${userManagementTab === 'processed' ? 'bg-[#002D62] text-white dark:bg-blue-600' : 'hover:bg-gray-100 dark:hover:bg-slate-800'}`}
                ...
                <button
                  ...
                  className={`text-left rtl:text-right px-4 py-3 rounded-lg font-medium transition-colors ${userManagementTab === 'deleted' ? 'bg-[#002D62] text-white dark:bg-blue-600' : 'hover:bg-gray-100 dark:hover:bg-slate-800'}`}
                    ...
                    <thead>
                      <tr className="border-b" style={{ backgroundColor: tableHeaderBg, borderColor: borderColor, color: textMuted }}>
                        ...
                        <th className="p-3 align-top">
                          <div className="font-semibold mb-2">{language === "ar" ? "إجراءات" : "Actions"}</div>
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={u.id} className="border-b transition-colors" style={{ borderColor: borderColor, color: textColor }}>
                          {editingUserId === u.id ? (
                            <>
                              ...
                              <td className="p-3 flex gap-2">
                                <button
                                  ...
                                  className="text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded"
                                >
                                  Save
                                </button>
                                <button onClick={() => setEditingUserId(null)} className="text-gray-600 bg-gray-50 dark:text-gray-300 dark:bg-gray-800 px-3 py-1 rounded">
                                  Cancel
                                </button>
                              </td>
                            </>
                            ...
                            <>
                              ...
                              <td className="p-3 flex gap-2">
                                <button onClick={() => handleApprove(u.id)} className="flex items-center text-green-600 bg-green-50 dark:bg-green-900/30 px-3 py-1 rounded hover:opacity-80">
                                  {language === "ar" ? "موافق" : "Approve"}
                                </button>
                                <button onClick={() => { setEditingUserId(u.id); setEditFormData(u); }} className="flex items-center text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded hover:opacity-80">
                                  {language === "ar" ? "تعديل" : "Edit"}
                                </button>
                                <button onClick={() => handleReject(u.id)} className="flex items-center text-red-600 bg-red-50 dark:bg-red-900/30 px-3 py-1 rounded hover:opacity-80">
                      ...
                      {users.filter(u => (u.status === "approved" || u.status === "rejected") && u.createdAt).map((u) => (
                        <tr key={u.id} className="border-b transition-colors" style={{ borderColor: borderColor, color: textColor }}>
                          {editingUserId === u.id ? (
                            <>
                              <td className="p-3">
                                <input type="text" value={editFormData.hrCode || ""} onChange={(e) => setEditFormData({ ...editFormData, hrCode: e.target.value })} className="border rounded px-2 py-1 w-24" style={{ backgroundColor: inputBg, borderColor: borderColor, color: textColor }} />
                              </td>
                              <td className="p-3">
                                <input type="text" value={editFormData.name || ""} onChange={(e) => setEditFormData({ ...editFormData, name: e.target.value })} className="border rounded px-2 py-1 w-32" style={{ backgroundColor: inputBg, borderColor: borderColor, color: textColor }} />
                              </td>
                              <td className="p-3">
                                <input type="text" value={editFormData.department || ""} onChange={(e) => setEditFormData({ ...editFormData, department: e.target.value })} className="border rounded px-2 py-1 w-32" style={{ backgroundColor: inputBg, borderColor: borderColor, color: textColor }} />
                              </td>
                              <td className="p-3">
                                <select value={editFormData.role || "trainee"} onChange={(e) => setEditFormData({ ...editFormData, role: e.target.value as any })} className="border rounded px-2 py-1" style={{ backgroundColor: inputBg, borderColor: borderColor, color: textColor }}>
                                  <option value="trainee">Trainee</option>
                                  <option value="manager">Manager</option>
                              ...
                              <td className="p-3">
                                <DataField>{new Date(u.createdAt!).toLocaleString()}</DataField>
                              </td>
                              <td className="p-3 flex gap-2">
                                <button onClick={() => { setUsers(users.map((user) => user.id === u.id ? { ...user, ...editFormData } : user)); setEditingUserId(null); }} className="text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded">
                            ...
                            <>
                              ...
                              <td className="p-3 flex gap-2">
                                {u.status === "approved" && (
                                  <>
                                    <button onClick={() => { setEditingUserId(u.id); setEditFormData(u); }} className="text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded hover:opacity-80">
                                      {language === "ar" ? "تعديل" : "Edit"}
                                    </button>
                    ...
                    <tbody>
                      {users.filter(u => u.status === "deleted").map(u => (
                        <tr key={u.id} className="border-b opacity-80" style={{ backgroundColor: isDark ? 'rgba(153, 27, 27, 0.1)' : '#fef2f2', borderColor: borderColor, color: textColor }}>
                          ...
                          <td className="p-3">
                            <button onClick={() => handleRestoreUser(u.id)} className="flex items-center text-green-600 bg-green-100 dark:bg-green-900/30 px-3 py-1 rounded hover:opacity-80">
          ...
          <div className="w-full overflow-hidden">
              <div className="flex flex-wrap justify-between items-center mb-6 gap-3">
                ...
                {hasActiveFilters && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer animate-fade-in"
                  >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6 print:hidden">
              <div className="p-4 rounded-lg border shadow-sm flex flex-col items-center justify-center text-center transition-colors duration-300" style={{ backgroundColor: cardColor, borderColor: borderColor }}>
                ...
                <span className="text-xs font-semibold mb-1" style={{ color: textMuted }}>
                  {language === "ar" ? "إجمالي الدورات" : "Total Sessions"}
                </span>
              ...
              <div className="p-4 rounded-lg border shadow-sm flex flex-col items-center justify-center text-center transition-colors duration-300" style={{ backgroundColor: cardColor, borderColor: borderColor }}>
                ...
                <span className="text-xs font-semibold mb-1" style={{ color: textMuted }}>
                  {language === "ar" ? "المهندسين" : "Total Engineers"}
                </span>
                <span className="text-xl font-bold" style={{ color: textColor }}>
              ...
              <div className="p-4 rounded-lg border shadow-sm flex flex-col items-center justify-center text-center transition-colors duration-300" style={{ backgroundColor: cardColor, borderColor: borderColor }}>
                ...
                <span className="text-xs font-semibold mb-1" style={{ color: textMuted }}>
                  {language === "ar" ? "الفنيين" : "Total Technicians"}
                </span>
                <span className="text-xl font-bold" style={{ color: textColor }}>
              ...
              <div className="p-4 rounded-lg border shadow-sm flex flex-col items-center justify-center text-center transition-colors duration-300" style={{ backgroundColor: cardColor, borderColor: borderColor }}>
                ...
                <span className="text-xs font-semibold mb-1" style={{ color: textMuted }}>
                  {language === "ar" ? "المشغلين" : "Total Operators"}
                </span>
                <span className="text-xl font-bold" style={{ color: textColor }}>
                    ...
                    <tr className="border-b" style={{ backgroundColor: tableHeaderBg, borderColor: borderColor }}>
                      <th className="p-3">
                        ...
                        <div className="relative">
                          ...
                          />
                          {searchHrCode && (
                            <button onClick={() => setSearchHrCode("")} className="absolute right-1 top-1/2 -translate-y-1/2" style={{ color: textMuted }}>
                              <X size={12} />
                            </button>
                          )}
                        </div>
                      ...
                      <th className="p-3">
                        ...
                        <div className="relative">
                          ...
                          />
                          {searchTrainee && (
                            <button onClick={() => setSearchTrainee("")} className="absolute right-1 top-1/2 -translate-y-1/2" style={{ color: textMuted }}>
                              <X size={12} />
                            </button>
                          )}
                        </div>
                      ...
                      <th className="p-3">
                        ...
                        <div className="relative">
                          ...
                          {searchDepartment && (
                            <button onClick={() => setSearchDepartment("")} className="absolute right-4 top-1/2 -translate-y-1/2" style={{ color: textMuted }}>
                              <X size={12} />
                            </button>
                          )}
                        </div>
                      ...
                      <th className="p-3">
                        ...
                        <div className="relative">
                          ...
                          {selectedCourseFilter && (
                            <button onClick={() => setSelectedCourseFilter("")} className="absolute right-4 top-1/2 -translate-y-1/2" style={{ color: textMuted }}>
                              <X size={12} />
                            </button>
                          )}
                        </div>
                      ...
                      <th className="p-3 align-top">
                        <div className="font-semibold mb-2" style={{ color: textMuted }}>{language === "ar" ? "المدة" : "Duration"}</div>
                      </th>
                      <th className="p-3 align-top">
                        <div className="font-semibold mb-2" style={{ color: textMuted }}>{language === "ar" ? "أيام الحضور" : "Attended Days"}</div>
                      </th>
                      <th className="p-3 align-top">
                        <div className="font-semibold mb-2" style={{ color: textMuted }}>{language === "ar" ? "الدرجة" : "Score"}</div>
                      </th>
                      <th className="p-3 align-top min-w-[140px]">
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="relative">
                            ...
                            />
                            {fromDateFilter && (
                              <button onClick={() => setFromDateFilter("")} className="absolute right-1 top-1/2 -translate-y-1/2" style={{ color: textMuted }}>
                                <X size={12} />
                              </button>
                            )}
                          </div>
                          <div className="relative">
                            ...
                            />
                            {toDateFilter && (
                              <button onClick={() => setToDateFilter("")} className="absolute right-1 top-1/2 -translate-y-1/2" style={{ color: textMuted }}>
                                <X size={12} />
                              </button>
                            )}
                          </div>
                      ...
                      <th className="p-3 align-top">
                        <div className="font-semibold mb-2" style={{ color: textMuted }}>{language === "ar" ? "إجراءات" : "Actions"}</div>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredRecords.map((r) => {
                      const user = users.find((u) =>
                        u.id === r.userId || u.hrCode === r.hrCode || u.hrCode === r.userId || u.hrCode === `HR${r.userId}` || u.name?.trim().toLowerCase() === (r.traineeName || r.userId || "").trim().toLowerCase()
                      );
                      ...
                      const traineeImage = user?.profileImageUrl || r.raw?.["Profile Image"] || r.raw?.["Photo"] || r.raw?.["Image"] || r.raw?.["صورة"];
                      ...
                      return (
                        <tr key={r.id} className="border-b last:border-0 hover:opacity-80 transition-colors" style={{ borderColor: borderColor }}>
                          ...
                          <td className="p-3 font-medium">
                            <TraineeAvatarWithName name={traineeDisplayName} imageUrl={traineeImage} hrCode={traineeHrCode} />
                          </td>
                          ...
                          <td className="p-3" style={{ color: textMuted }}><DataField>{course?.duration || cleanedDuration || "N/A"}</DataField></td>
                          ...
                          <td className="p-3 text-center">
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                if (window.confirm(language === "ar" ? "هل أنت متأكد من حذف هذا السجل نهائياً؟" : "Are you sure you want to permanently delete this record?")) {
                                  try {
                              ...
                              className="text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/30 p-1.5 rounded-lg transition-colors cursor-pointer inline-flex items-center justify-center"
                            ...
                            <button
                              ...
                              className="text-blue-600 hover:text-blue-800 hover:bg-blue-50 dark:hover:bg-blue-900/30 p-1.5 rounded-lg transition-colors cursor-pointer inline-flex items-center justify-center"
                    ...
                    {filteredRecords.length === 0 && (
                      <tr>
                        <td colSpan={9} className="p-8 text-center" style={{ color: textMuted }}>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <div className="border rounded-lg p-6 shadow-sm flex flex-col items-center justify-center transition-colors" style={{ backgroundColor: cardColor, borderColor: borderColor }}>
                <span className="text-sm font-semibold uppercase tracking-wider mb-2" style={{ color: textMuted }}>
                  {language === "ar" ? "المتدربين الفريدين" : "Unique Trainees"}
                </span>
              ...
              <div className="border rounded-lg p-6 shadow-sm flex flex-col items-center justify-center transition-colors" style={{ backgroundColor: cardColor, borderColor: borderColor }}>
                <span className="text-sm font-semibold uppercase tracking-wider mb-2" style={{ color: textMuted }}>
                  {language === "ar" ? "الدورات المختلفة" : "Distinct Courses"}
                </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="mt-12 border-t pt-8" style={{ borderColor: borderColor }}>
                <h3 className="font-bold text-xl mb-6" style={{ color: isDark ? '#60a5fa' : '#002D62' }}>
                  {language === "ar" ? "مخططات متقدمة" : "Advanced Charts"}
                </h3>
        ...
        {["tools", "tools_manage", "tools_create", "tools_reports", "tools_logs", "tools_usage"].includes(currentView) && (
          <div className="space-y-12">
            ...
            {["tools", "tools_manage"].includes(currentView) && (
              <>
            <div>
              <h2 className="text-2xl font-bold mb-6 border-l-4 border-[#FFC000] pl-3 rtl:pr-3 rtl:pl-0 rtl:border-r-4 rtl:border-l-0" style={{ color: isDark ? '#60a5fa' : '#002D62' }}>
                {language === "ar" ? "إعلانات الجلسات" : "Session Announcements"}
              </h2>
            ...
            {reminderToast && (
              <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-[9999] w-[90%] max-w-sm bg-emerald-50 dark:bg-emerald-900/80 border-l-4 border-emerald-500 p-4 rounded shadow-sm text-emerald-800 dark:text-emerald-200 flex items-center justify-between transition-all animate-fadeIn">
                <div className="flex items-center gap-2">
                  <Bell className="h-5 w-5 text-emerald-600 dark:text-emerald-400 animate-bounce" />
                  <span className="font-semibold text-sm md:text-base">{reminderToast}</span>
                </div>
                <button onClick={() => setReminderToast(null)} className="text-emerald-600 dark:text-emerald-400 hover:opacity-75 font-bold text-sm">
                  <X size={16} />
                </button>
              </div>
            )}
            ...
            <div className={currentView === "tools_manage" ? "grid grid-cols-1 gap-8" : "grid grid-cols-1 lg:grid-cols-2 gap-8"}>
              ...
              <div>
                <h2 className="text-xl font-semibold mb-4" style={{ color: textColor }}>
                  {editingSessionId ? (language === "ar" ? "تعديل الجلسة" : "Edit Session") : (language === "ar" ? "إنشاء جلسة جديدة" : "Create New Session")}
                </h2>
              ...
              <div>
                <h2 className="text-xl font-semibold mb-4" style={{ color: textColor }}>
                  {language === "ar" ? "إدارة الجلسات القادمة" : "Manage Upcoming Sessions"}
                </h2>
                {upcomingSessions.length === 0 ? (
                  <div className="text-center py-12 px-4 border border-dashed rounded-lg transition-colors" style={{ backgroundColor: tableHeaderBg, borderColor: borderColor }}>
                    ...
                    <p className="font-medium" style={{ color: textMuted }}>
        ...
        {["tools", "tools_reports"].includes(currentView) && (
          <>
            <div className="border-t pt-8 mt-8" style={{ borderColor: borderColor }}>
              <h2 className="text-2xl font-bold mb-6 border-l-4 border-[#FFC000] pl-3 rtl:pr-3 rtl:pl-0 rtl:border-r-4 rtl:border-l-0" style={{ color: isDark ? '#60a5fa' : '#002D62' }}>
                {language === "ar" ? "تحليل الاحتياجات التدريبية" : "Training Needs Analysis"}
              </h2>
          ...
          {["tools", "tools_reports"].includes(currentView) && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t pt-8 mt-8" style={{ borderColor: borderColor }}>
              ...
              <div className="max-w-md">
                <h2 className="text-2xl font-bold mb-6 border-l-4 border-[#FFC000] pl-3 rtl:pr-3 rtl:pl-0 rtl:border-r-4 rtl:border-l-0" style={{ color: isDark ? '#60a5fa' : '#002D62' }}>
                  {language === "ar" ? "مشاركة الموارد" : "Resource Sharing"}
                </h2>
                <form onSubmit={handleShareResource} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: textMuted }}>
                      {language === "ar" ? "رابط جوجل درايف" : "Google Drive Link"}
                    </label>
              ...
              <div>
                <div className="flex flex-wrap justify-between items-center mb-6 gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {user?.role === 'admin' && (
                      <>
                        <button onClick={() => setShowGlobalAnnouncement(true)} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm text-sm">
                          <Globe size={18} />
                          {language === 'ar' ? 'إعلان عام' : 'Global Broadcast'}
                        </button>
                        <button onClick={() => setShowAnnouncementManager("GLOBAL")} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm text-sm">
                          <Megaphone size={18} />
                          {language === 'ar' ? 'إدارة الإعلانات' : 'Manage Announcements'}
                        </button>
                      </>
                    )}
                    <button onClick={() => setShowMonthlyReport(true)} className="flex items-center gap-2 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm text-sm" style={{ backgroundColor: isDark ? '#2563eb' : '#002D62' }}>
                      <Mail size={18} />
                      {language === 'ar' ? 'تقرير التحديث الشهري' : 'Monthly Update Report'}
                    </button>
                    <button onClick={() => exportCloudBackup(users, records, upcomingSessions, cleanedData || [])} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm text-sm">
                      <Download size={18} />
                ...
                <p className="mb-6" style={{ color: textMuted }}>
                  ...
                  <br />
                  <span className="px-1 py-0.5 rounded text-sm mt-1 inline-block" style={{ backgroundColor: tableHeaderBg, color: textColor }}>
                ...
                {syncSuccess && (
                  <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 px-4 py-3 rounded mb-6 flex items-center">
                    ...
                    <span className="font-medium">
                      {language === "ar" ? "تمت المزامنة بنجاح!" : "Data successfully synced from OneDrive!"}
                    </span>
                  </div>
                )}
                <div className="mb-6">
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none rtl:left-auto rtl:right-0 rtl:pl-0 rtl:pr-3">
                      <Database size={18} style={{ color: textMuted }} />
                    </div>
                ...
                {isSyncing && (
                  <div className="mb-6">
                    <div className="flex justify-between text-sm mb-1" style={{ color: textMuted }}>
                      <span>{language === "ar" ? "جاري جلب ومزامنة البيانات..." : "Fetching & Syncing..."}</span>
                      <span>{syncProgress}%</span>
                    </div>
                    <div className="w-full rounded-full h-2.5" style={{ backgroundColor: tableHeaderBg }}>
                      <div className="h-2.5 rounded-full transition-all duration-300" style={{ width: `${syncProgress}%`, backgroundColor: isDark ? '#3b82f6' : '#002D62' }}></div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center my-6">
                  ...
                  <span className="mx-4 text-sm font-medium" style={{ color: textMuted }}>
                    {language === "ar" ? "أو" : "OR"}
                  </span>
                  <div className="flex-grow border-t" style={{ borderColor: borderColor }}></div>
                </div>
        ...
        {currentView === "tools_usage" && (
          <FirebaseUsageModal onClose={() => setCurrentView("dashboard")} />
        )}
        {/* --- END tools_usage --- */}
      </div>
      ...
      {showGlobalAnnouncement && (
        <AnnouncementModal onClose={() => setShowGlobalAnnouncement(false)} />
      )}
      {qrSession && (
        <QRCodeModal session={qrSession} language={language} onClose={() => setQrSession(null)} />
      )}
      {announcingSession && (
        <AnnouncementModal session={announcingSession} onClose={() => setAnnouncingSession(null)} />
      )}
      {showAnnouncementManager && (
        <AnnouncementManagerModal sessionId={showAnnouncementManager === "GLOBAL" ? undefined : showAnnouncementManager} onClose={() => setShowAnnouncementManager(null)} />
      )}
      {editingRecord && (
        <EditRecordModal record={editingRecord} onClose={() => setEditingRecord(null)} />
      )}
      {showUsageModal && (
        <FirebaseUsageModal onClose={() => setShowUsageModal(false)} />
      )}
      {showMonthlyReport && (
        <MonthlyReportModal onClose={() => setShowMonthlyReport(false)} records={records} upcomingSessions={upcomingSessions} />
      )}
    </div>
  );
};