...
import { Bell, Share2, Users, FileText, CheckCircle2, Database, UploadCloud, RefreshCw, CheckCircle, BookOpen, Calendar, HardHat, Wrench, Settings, Printer, X, Download, Pencil, Trash2, RotateCcw, Check, ChevronDown, ChevronUp, Clock, AlertTriangle, Ban } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  const { t, language, user, users, setUsers, records, setRecords, upcomingSessions, setUpcomingSessions, addUpcomingSession, updateUpcomingSession, deleteUpcomingSession, restoreUpcomingSession, cancelSession, reactivateSession } = useAppContext();
  ...
  const dynamicCourses = useMemo(() => {
    ...
    records.forEach(r => {
      if (r.courseId) {
        map.set(r.courseId, r.courseName || mockCourses.find(c => c.id === r.courseId)?.title || r.courseId);
      }
    });
  ...
  const departmentStats = useMemo(() => {
    ...
    return Object.entries(stats).map(([name, set]) => ({
      department: name,
      trainees: set.size
  ...
  const handleSendReminder = (sessionId: string, reminderType: 'Standard' | 'Final' = 'Standard') => {
    console.log("Sending reminder for session:", sessionId, "Type:", reminderType);
    ...
    const typeLabel = isFinal 
      ? (language === 'ar' ? 'التذكير النهائي' : 'FINAL REMINDER') 
      : (language === 'ar' ? 'تذكير عادي' : 'Standard reminder');
  ...
  const handleShareResource = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`Shared ${resourceLink} with attendees of course ${selectedCourseForResource}`);
    setResourceLink('');
  };
  ...
  const getAdminReportOptions = (): ReportOptions => {
    ...
    return {
      ...
      fileName: isSingleTraineeFiltered ? `Training_Report_${singleTrainee?.hrCode || 'Individual'}.pdf` : 'OED_Technical_Training_Report.pdf',
    };
  };
    ...
    reader.onload = (evt) => {
      try {
        ...
        for (let c = 6; c < maxCols; c++) {
          if (courseRow[c] && typeof courseRow[c] === 'string' && courseRow[c].trim() !== '') {
          ...
          for (let c = 6; c < maxCols; c += 4) {
            ...
            if (dateVal) {
              ...
              if (dateVal instanceof Date) {
                ...
                formattedDate = new Date(Math.round((dateVal - 25569) * 86400 * 1000)).toISOString().split('T')[0];
              }
              ...
              } else if (typeof scoreVal === 'string' && !scoreVal.includes('%')) {
        ...
        setUsers(prev => {
          const uniqueNewUsers = newUsers.filter(nu => !prev.some(u => u.id === nu.id));
  ...
  const filteredRecords = records.filter(r => {
    ...
    if (searchHrCode && !user?.hrCode?.toLowerCase().includes(searchHrCode.toLowerCase())) return false;
    if (searchTrainee && !user?.name?.toLowerCase().includes(searchTrainee.toLowerCase())) return false;
    if (searchDepartment && user?.department !== searchDepartment) return false;
    if (selectedCourseFilter && r.courseId !== selectedCourseFilter) return false;
  ...
  const uniqueTraineeHrCodes = useMemo(() => Array.from(new Set(filteredRecords.map(r => {
    const u = users.find(u => u.id === r.userId);
    return u?.hrCode;
  }).filter(Boolean))), [filteredRecords, users]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 print:p-0">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 min-h-[400px]">
                  ...
                  <tbody>
                    {pendingUsers.map(u => (
                      <tr key={u.id} className="border-b">
                        <td className="p-3"><DataField>{u.username || u.hrCode}</DataField></td>
                        ...
                        <td className="p-3 flex gap-2">
                          ...
                          >
                            <CheckCircle2 size={16} className="mr-1 rtl:ml-1 rtl:mr-0" /> {t('approve')}
                          </button>
                          <button 
        ...
        {activeTab === 'records' && (
          <div>
            <div className="mb-6 flex justify-between items-center print:hidden">
              <h2 className="text-xl font-semibold text-gray-800">{t('globalRecords')}</h2>
              <div className="flex items-center gap-3">
                <button 
                  onClick={resetFilters} 
                  className="flex items-center justify-center bg-gray-100 text-gray-700 px-3 py-2 rounded border border-gray-300 hover:bg-gray-200 transition-colors text-sm"
                >
                  <RefreshCw size={16} className="mr-1.5 rtl:ml-1.5 rtl:mr-0" />
                  {language === 'ar' ? 'مسح الفلاتر' : 'Reset Filters'}
                </button>
                <button 
                  onClick={handlePrint} 
                  className="bg-[#002D62] text-white px-3 py-2 rounded flex items-center gap-1.5 shadow hover:bg-blue-900 transition-colors text-sm font-medium"
                >
                  <Printer size={16} />
                  {isSingleTraineeFiltered ? (language === 'ar' ? 'طباعة التقرير' : 'Print Report') : (language === 'ar' ? 'طباعة التقرير' : 'Print Report')}
                </button>
                <button 
                  onClick={handleDownloadPDF} 
                  className="bg-emerald-700 text-white px-3 py-2 rounded flex items-center gap-1.5 shadow hover:bg-emerald-800 transition-colors text-sm font-medium"
                >
                  <Download size={16} />
            ...
            <div className="hidden print:block text-black bg-white">
              ...
              <div className="mb-6">
                <h1 className="text-2xl font-bold text-center uppercase tracking-wider mb-2 text-gray-800">
                  {isSingleTraineeFiltered ? (language === 'ar' ? 'تقرير تدريب فردي' : 'Individual Trainee Training Report') : (language === 'ar' ? 'تقرير التدريب الفني' : 'Technical Training Report')}
                </h1>
                ...
                {singleTrainee && (
                  <div className="bg-gray-50 border-2 border-gray-200 p-5 rounded-lg flex flex-wrap justify-between gap-4">
                    <div className="flex-1 min-w-[200px]">
                      ...
                      <span className="text-lg font-bold text-[#002D62]">{singleTrainee.name}</span>
                    </div>
                    <div className="flex-1 min-w-[150px]">
                      <span className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">{t('hrCode')}</span> 
                      <span className="text-lg font-bold text-gray-800">{singleTrainee.hrCode}</span>
                    </div>
                    <div className="flex-1 min-w-[200px]">
                      <span className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">{t('department')}</span> 
                      <span className="text-lg font-medium text-gray-800">{singleTrainee.department}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <table className="w-full text-left border-collapse text-sm print:table mb-12">
                ...
                <tbody className="print:table-row-group">
                  {filteredRecords.map((r, idx) => {
                    ...
                    const displayCourseTitle = course?.title || r.courseName || 'Unknown';
                    const rawDuration = r.raw?.['Course Duration'] || r.totalDays;
                    const cleanedDuration = rawDuration ? `${rawDuration}` : '';
                    return (
                      <tr key={r.id} className="border-b border-gray-300 break-inside-avoid">
                        <td className="p-3 font-medium text-gray-900">{displayCourseTitle}</td>
                        <td className="p-3 text-gray-700">{course?.duration || cleanedDuration || 'N/A'}</td>
                        <td className="p-3 text-gray-700">{r.raw?.['Attended Days'] || r.daysAttended}</td>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6 print:hidden">
              <div className="glass-card p-4">
                ...
                <span className="text-xs text-gray-500 font-semibold mb-1">{language === 'ar' ? 'إجمالي الكورسات' : 'Total Courses'}</span>
                <span className="text-xl font-bold text-[#002D62]">{kpiStats.totalCourses}</span>
              </div>
              <div className="glass-card p-4">
                <Calendar className="text-[#FFC000] mb-2" size={24} />
                <span className="text-xs text-gray-500 font-semibold mb-1">{language === 'ar' ? 'إجمالي انعقاد الدورات' : 'Total Sessions'}</span>
                <span className="text-xl font-bold text-[#002D62]">{kpiStats.totalSessions}</span>
              </div>
              <div className="glass-card p-4">
                <Users className="text-green-600 mb-2" size={24} />
                <span className="text-xs text-gray-500 font-semibold mb-1">{language === 'ar' ? 'إجمالي المتدربين' : 'Total Participants'}</span>
                <span className="text-xl font-bold text-[#002D62]">{kpiStats.totalParticipants}</span>
              </div>
              <div className="glass-card p-4">
                <HardHat className="text-blue-500 mb-2" size={24} />
                <span className="text-xs text-gray-500 font-semibold mb-1">{language === 'ar' ? 'إجمالي المهندسين' : 'Total Engineers'}</span>
                <span className="text-xl font-bold text-[#002D62]">{kpiStats.totalEngineers}</span>
              </div>
              <div className="glass-card p-4">
                <Wrench className="text-purple-500 mb-2" size={24} />
                <span className="text-xs text-gray-500 font-semibold mb-1">{language === 'ar' ? 'إجمالي الفنيين' : 'Total Technicians'}</span>
                <span className="text-xl font-bold text-[#002D62]">{kpiStats.totalTechnicians}</span>
              </div>
              <div className="glass-card p-4">
                <Settings className="text-gray-500 mb-2" size={24} />
                <span className="text-xs text-gray-500 font-semibold mb-1">{language === 'ar' ? 'إجمالي المشغلين' : 'Total Operators'}</span>
                <span className="text-xl font-bold text-[#002D62]">{kpiStats.totalOperators}</span>
              </div>
            </div>
            ...
            {selectedCourseFilter && selectedCourseDetails ? (
              <div className="mb-6 print:hidden">
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4 flex flex-wrap gap-4 items-center">
                  <h3 className="text-lg font-bold text-[#002D62]"><DataField>{selectedCourseDetails.title}</DataField></h3>
                  ...
                  {courseSessions.map(date => {
                    ...
                    return (
                      <div key={date} className="border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        >
                          <span className="font-bold text-gray-700">{formatDateToStandard(date)}</span>
                          <span className="text-sm text-gray-500">{attendeesOnDate.length} {t('attendees')}</span>
                        </button>
                            ...
                            <table className="w-full text-left border-collapse text-sm">
                              <thead>
                                <tr className="border-b text-gray-600">
                                  ...
                                  <th className="pb-2 font-medium">{t('department')}</th>
                                ...
                                {attendeesOnDate.map(r => {
                                  ...
                                  return (
                                    <tr key={r.id} className="border-b last:border-0 hover:bg-gray-50">
                                      <td className="py-2"><DataField>{u?.name}</DataField></td>
                                      <td className="py-2 text-gray-600"><DataField>{u?.department}</DataField></td>
                                      <td className="py-2 font-bold text-[#002D62]">{r.score}</td>
                                    </tr>
                                  );
                                })}
                              </tbody>
                ...
                <table className="w-full text-left border-collapse text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="p-3">
                        <div className="font-semibold text-gray-700 mb-2">{language === 'ar' ? 'الرقم الوظيفي' : 'HR Code'}</div>
                        <div className="relative">
                          <input type="text" value={searchHrCode} onChange={e => setSearchHrCode(e.target.value)} className="w-full border rounded px-2 py-1 text-xs focus:ring-[#002D62] pr-6" placeholder="Filter..." />
                          {searchHrCode && <button onClick={() => setSearchHrCode('')} className="absolute right-1 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700"><X size={12} /></button>}
                        </div>
                      </th>
                      <th className="p-3">
                        <div className="font-semibold text-gray-700 mb-2">{t('name')}</div>
                        <div className="relative">
                          <input type="text" value={searchTrainee} onChange={e => setSearchTrainee(e.target.value)} className="w-full border rounded px-2 py-1 text-xs focus:ring-[#002D62] pr-6" placeholder="Filter..." />
                          {searchTrainee && <button onClick={() => setSearchTrainee('')} className="absolute right-1 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700"><X size={12} /></button>}
                        </div>
                      </th>
                      <th className="p-3">
                        <div className="font-semibold text-gray-700 mb-2">{t('department')}</div>
                        <div className="relative">
                          <select value={searchDepartment} onChange={e => setSearchDepartment(e.target.value)} className="w-full border rounded px-2 py-1 text-xs focus:ring-[#002D62] appearance-none pr-6">
                            <option value="">All</option>
                            {dynamicDepartments.map(d => <option key={d} value={d}>{d}</option>)}
                          </select>
                          {searchDepartment && <button onClick={() => setSearchDepartment('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700"><X size={12} /></button>}
                        </div>
                      </th>
                      <th className="p-3">
                        <div className="font-semibold text-gray-700 mb-2">{t('courseName')}</div>
                        <div className="relative">
                          <select value={selectedCourseFilter} onChange={e => setSelectedCourseFilter(e.target.value)} className="w-full border rounded px-2 py-1 text-xs focus:ring-[#002D62] appearance-none pr-6">
                            <option value="">All</option>
                            {dynamicCourses.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
                          </select>
                          {selectedCourseFilter && <button onClick={() => setSelectedCourseFilter('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700"><X size={12} /></button>}
                        </div>
                      </th>
                      <th className="p-3 align-top">
                        <div className="font-semibold text-gray-700 mb-2">{language === 'ar' ? 'مدة الدورة' : 'Duration'}</div>
                      </th>
                      <th className="p-3 align-top">
                        <div className="font-semibold text-gray-700 mb-2">{language === 'ar' ? 'أيام الحضور' : 'Attended Days'}</div>
                      </th>
                      <th className="p-3 align-top">
                        <div className="font-semibold text-gray-700 mb-2">{t('score')}</div>
                      </th>
                      <th className="p-3 align-top min-w-[140px]">
                        <div className="font-semibold text-gray-700 mb-2">{t('date')}</div>
                        <div className="flex flex-col gap-2">
                          <div className="relative">
                            <input type="date" value={fromDateFilter} onChange={e => setFromDateFilter(e.target.value)} className="w-full border rounded px-2 py-1 text-xs focus:ring-[#002D62] pr-6" title={language === 'ar' ? 'من تاريخ' : 'From Date'} />
                            {fromDateFilter && <button onClick={() => setFromDateFilter('')} className="absolute right-1 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 bg-white"><X size={12} /></button>}
                            {fromDateFilter && <div className="text-[10px] text-[#002D62] font-semibold mt-0.5">{formatDateToStandard(fromDateFilter)}</div>}
                          </div>
                          <div className="relative">
                            <input type="date" value={toDateFilter} onChange={e => setToDateFilter(e.target.value)} className="w-full border rounded px-2 py-1 text-xs focus:ring-[#002D62] pr-6" title={language === 'ar' ? 'إلى تاريخ' : 'To Date'} />
                            {toDateFilter && <button onClick={() => setToDateFilter('')} className="absolute right-1 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 bg-white"><X size={12} /></button>}
                  ...
                  <tbody>
                    {filteredRecords.map(r => {
                      ...
                      return (
                        <tr key={r.id} className="border-b last:border-0 hover:bg-gray-50">
                          <td className="p-3 font-medium text-gray-800"><DataField>{user?.hrCode || r.hrCode}</DataField></td>
                          <td className="p-3 font-medium text-[#002D62]"><DataField>{user?.name || r.userId}</DataField></td>
                          <td className="p-3 text-gray-600"><DataField>{user?.department}</DataField></td>
                          <td className="p-3"><DataField>{displayCourseTitle}</DataField></td>
                          <td className="p-3 text-gray-600"><DataField>{course?.duration || cleanedDuration || 'N/A'}</DataField></td>
                          <td className="p-3 text-gray-600"><DataField>{r.raw?.['Attended Days'] || r.daysAttended}</DataField></td>
                          <td className="p-3 font-bold text-gray-800"><DataField>{formatScore(r.raw?.['Score'] || r.score)}</DataField></td>
                          <td className="p-3 text-gray-500"><DataField>{formatDateToStandard(r.attendanceDate)}</DataField></td>
                        </tr>
                      );
                    })}
        ...
        {activeTab === 'analytics' && (
          <div>
            <h2 className="text-xl font-semibold mb-6 text-gray-800">{t('analytics')}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm flex flex-col items-center justify-center">
                <span className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-2">{language === 'ar' ? 'إجمالي السجلات' : 'Total Records'}</span>
                <span className="text-3xl font-bold text-[#002D62]">{records.length}</span>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm flex flex-col items-center justify-center">
                <span className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-2">{language === 'ar' ? 'إجمالي المتدربين' : 'Unique Trainees'}</span>
                <span className="text-3xl font-bold text-[#FFC000]">{totalUniqueTrainees}</span>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm flex flex-col items-center justify-center">
                <span className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-2">{language === 'ar' ? 'الدورات المتميزة' : 'Distinct Courses'}</span>
                <span className="text-3xl font-bold text-green-600">{totalDistinctCourses}</span>
              </div>
            </div>
                    ...
                    return (
                      <div key={idx}>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="font-medium text-gray-700 truncate mr-4"><DataField>{stat.courseName}</DataField></span>
                          <span className="text-gray-900 font-bold">{stat.attendees}</span>
                        </div>
                        <div className="w-full bg-gray-100 rounded-full h-2.5">
                          <div className="bg-[#002D62] h-2.5 rounded-full" style={{ width: `${percent}%` }}></div>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="overflow-y-auto flex-1 pt-3 pr-1 space-y-4">
                  {departmentStats.map((stat, idx) => {
                    ...
                    return (
                      <div key={idx}>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="font-medium text-gray-700 truncate mr-4"><DataField>{stat.department}</DataField></span>
                          <span className="text-gray-900 font-bold">{stat.trainees}</span>
                        </div>
                        <div className="w-full bg-gray-100 rounded-full h-2.5">
                          <div className="bg-[#FFC000] h-2.5 rounded-full" style={{ width: `${percent}%` }}></div>
                        </div>
                      </div>
                    );
                  })}
        ...
        {activeTab === 'announcements' && (
          <div className="space-y-6">
            {reminderToast && (
              <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-[9999] w-[90%] max-w-sm bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded shadow-sm text-emerald-800 flex items-center justify-between transition-all animate-fadeIn">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm md:text-base">{reminderToast}</span>
                </div>
                <button onClick={() => setReminderToast(null)} className="text-emerald-600 hover:text-emerald-900 font-bold text-sm">
                  ✕
                </button>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <form onSubmit={handleCreateSession} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                      ...
                      <select required value={selectedCourseId} onChange={e => setSelectedCourseId(e.target.value)} className="w-full border rounded px-3 py-2 focus:ring-[#002D62] bg-white">
                    ...
                    <div>
                      ...
                      <input type="date" required value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full border rounded px-3 py-2 focus:ring-[#002D62]" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('endDate')}</label>
                      <input type="date" required value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full border rounded px-3 py-2 focus:ring-[#002D62]" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('sessionNumber')}</label>
                      <select required value={sessionNumber} onChange={e => setSessionNumber(e.target.value)} className="w-full border rounded px-3 py-2 focus:ring-[#002D62] bg-white">
                    ...
                    <div>
                      ...
                      <input type="time" required value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full border rounded px-3 py-2 focus:ring-[#002D62]" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('location')}</label>
                      <input type="text" required value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. Training Room - Central Workshop" className="w-full border rounded px-3 py-2 focus:ring-[#002D62]" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('targetParticipants')}</label>
                      <select required value={targetParticipants} onChange={e => setTargetParticipants(e.target.value)} className="w-full border rounded px-3 py-2 focus:ring-[#002D62] bg-white">
                  ...
                  <div className="flex gap-2 mt-6">
                    ...
                    {editingSessionId && (
                      <button type="button" onClick={handleCancelEdit} className="bg-gray-200 text-gray-700 font-bold py-3 px-4 rounded hover:bg-gray-300 transition-colors">
                        {t('cancelEdit')}
                      </button>
                    )}
                  </div>
              ...
              <div>
                <h2 className="text-xl font-semibold mb-4 text-gray-800">{t('manageUpcoming')}</h2>
                {upcomingSessions.length === 0 ? (
                  <div className="text-center py-12 px-4 bg-gray-50 border border-dashed border-gray-300 rounded-lg">
                    <Calendar className="mx-auto h-12 w-12 text-gray-400 mb-3" />
                    <p className="text-gray-600 font-medium">{t('noUpcomingSessions')}</p>
                  </div>
                ) : (
        ...
        {activeTab === 'tna' && (
          <div>
            <h2 className="text-xl font-semibold mb-6 text-gray-800">{t('tna')}</h2>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={tnaData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Bar dataKey="count" fill="#FFC000" radius={[4, 4, 0, 0]} name={t('requests')} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        {activeTab === 'resources' && (
          <div className="max-w-md">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">{t('resourceSharing')}</h2>
            <form onSubmit={handleShareResource} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('googleDriveLink')}</label>
                <input 
                  type="url" 
                  required 
                  value={resourceLink}
              ...
              <button type="submit" className="bg-[#002D62] text-white font-bold py-2 px-6 rounded hover:bg-blue-900 flex items-center">
                <Share2 size={18} className="mr-2 rtl:ml-2 rtl:mr-0" /> {t('share')}
              </button>
            </form>
          </div>
        )}
        ...
        {activeTab === 'sync' && (
          <div className="max-w-xl">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">
              {language === 'ar' ? 'تحديث البيانات من OneDrive' : 'Sync Excel Data to Database'}
            </h2>
            <p className="text-gray-600 mb-6">
              {language === 'ar' ? 'قم بتوفير رابط OneDrive لملف Excel الخاص بك لمزامنة سجلات التدريب.' : 'Provide a OneDrive link to your Excel file to synchronize training records.'}
              <br />
              Expected columns: <DataField className="bg-gray-100 px-1 py-0.5 rounded text-sm">ID, Prticipent Name, Department, Total Courses, Date 1, Duration 1, Scoor 1</DataField>
            </p>

            {syncSuccess && (
              <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded mb-6 flex items-center">
                <CheckCircle size={20} className="mr-2 rtl:ml-2 rtl:mr-0 flex-shrink-0" />
                <span className="font-medium">Data successfully synced from OneDrive!</span>
              </div>
            )}
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input 
                  type="url" 
                  placeholder={language === 'ar' ? 'الصق رابط OneDrive هنا...' : 'Paste your OneDrive Excel link here...'}
                  value={syncLink}
            ...
            {isSyncing && (
              <div className="mb-6">
                <div className="flex justify-between text-sm text-gray-600 mb-1">
                  <span>{language === 'ar' ? 'جاري الجلب والمزامنة...' : 'Fetching & Syncing...'}</span>
                  <span>{syncProgress}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2.5">
                  <div className="bg-[#002D62] h-2.5 rounded-full transition-all duration-300" style={{ width: `${syncProgress}%` }}></div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center my-6">
              ...
              <span className="mx-4 text-gray-500 text-sm font-medium">{language === 'ar' ? 'أو' : 'OR'}</span>
              <div className="flex-grow border-t border-gray-300"></div>
            </div>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 flex flex-col items-center justify-center bg-gray-50">
              ...
              {syncFile && (
                <p className="text-sm text-green-600 font-medium">Selected: {syncFile.name}</p>
              )}
            </div>
          </div>
        )}
