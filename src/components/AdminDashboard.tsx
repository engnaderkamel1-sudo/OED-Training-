...
`;
  ...
  const [drillDownModalType, setDrillDownModalType] = useState<null | 'courses' | 'sessions' | 'trainees' | 'engineers' | 'technicians' | 'operators' | 'interns'>(null);
  ...
  const dynamicCourses = useMemo(() => {
    ...
    (courses || []).forEach((c) => {
      ...
      if (title && title.length > 3 && !/^\d+$/.test(title) && !title.toLowerCase().includes('test') && !courseTitles.has(title.toLowerCase())) {
  ...
  }, [courses, upcomingSessions]);

  const dynamicDepartments = useMemo(() => {
    const depts = new Set<string>([
      "ORC - Katamia Central Workshop",
      "OC - Ain Sokhna Site Workshop",
      "OC - New Alamein Site Workshop",
      "ORC - Abu Rawash Workshop",
      "OC - Toshka Project Workshop",
      "OED - Technical Office & Engineering"
    ]);

    const invalidDepts = new Set(['ttms', 'asphalt plant', 'heavy machinery', 'general', 'test', 'n/a', 'undefined', 'civil works']);

    (users || []).forEach(u => {
      const d = (u.department || '').trim();
      if (d && d.length > 2 && !invalidDepts.has(d.toLowerCase()) && !/^\d+$/.test(d)) {
        depts.add(d);
      }
    });
  ...
  const courseStats = useMemo(() => {
    ...
    source.forEach(r => {
      ...
      if (cName && cName.length > 3 && !/^\d+$/.test(cName) && !cName.toLowerCase().includes('test')) {
  ...
  const departmentStats = useMemo(() => {
    if (user?.isDemoUser) {
      return EXECUTIVE_OED_DEPARTMENTS;
    }
    ...
    (users || []).forEach(u => {
      ...
      if (dept && dept.length > 2 && !invalidDepts.has(dept.toLowerCase()) && !/^\d+$/.test(dept)) {
...
<body style="font-family: Calibri, Arial, sans-serif; font-size: 11pt; color: #1e293b; line-height: 1.6;">
  ...
  const filteredRecords = useMemo(() => {
    ...
    const opUnique = new Set<string>();
    const internUnique = new Set<string>();
    let eng = 0, tech = 0, op = 0, intern = 0;
  ...
  const drillDownTraineesList = useMemo(() => {
    const map = new Map<string, { hrCode: string; name: string; roleCategory: 'engineer' | 'technician' | 'operator' | 'intern' | 'other'; department: string; courses: Set<string>; totalAttendances: number }>();
    (allRecordsPool || []).forEach(r => {
      const hrCode = (r.hrCode || r.userId || '').toString().trim();
      ...
      const normJob = `${u?.jobTitle || ''} ${r.role || ''} ${r.jobTitle || ''} ${dept}`.toLowerCase();
      let roleCategory: 'engineer' | 'technician' | 'operator' | 'intern' | 'other' = 'other';
      if (normJob.includes('intern') || normJob.includes('summer') || normJob.includes('طالب')) roleCategory = 'intern';
      else if (normJob.includes('eng') || normJob.includes('مهندس')) roleCategory = 'engineer';
        ...
        <div className="p-6 rounded-lg shadow-sm border min-h-[400px] transition-colors duration-300" style={{ backgroundColor: cardColor, borderColor: borderColor }}>
          ...
          {currentView === "userManagement" && (
            <UserManagementTab
              ...
              processedUpdatesList={processedUpdatesList}
              ...
              editingUpdateUserId={editingUpdateUserId}
              setEditingUpdateUserId={setEditingUpdateUserId}
              ...
              handleSaveUpdateEdit={handleSaveUpdateEdit}
            ...
            <div className="space-y-12">
              <div className="w-full overflow-hidden">
                ...
                {isSingleTraineeFiltered && singleTraineeProfile ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-6 print:hidden">
                    ...
                    {/* 7. Student Interns (Clickable Drill-Down) */}
                    <div 
                      onClick={() => { setDrillDownModalType('interns'); setDrillDownSearch(''); }}
                      role="button"
                      tabIndex={0}
                      title="Click to view all university student interns"
                      className="group p-4 rounded-2xl border shadow-xs flex flex-col items-center justify-between text-center transition-all duration-300 hover:shadow-lg hover:scale-[1.03] cursor-pointer hover:border-amber-400 dark:hover:border-amber-500" 
                      style={{ backgroundColor: cardColor, borderColor: borderColor }}
                    >
                      <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-950/50 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
        ...
        <div className="fixed inset-0 z-[99999] bg-black/75 backdrop-blur-sm flex items-center justify-center p-3 sm:p-5 animate-fade-in print:hidden">
          ...
          >
            ...
            <div className="bg-[#002D62] text-white p-5 sm:p-6 flex items-center justify-between border-b border-blue-950/50">
              <div className="flex items-center gap-3.5">
                <div className="w-12 h-12 rounded-2xl bg-amber-400 text-[#002D62] flex items-center justify-center font-black shadow-md">
                  ...
                  {drillDownModalType === 'interns' && <GraduationCap size={24} />}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-xl sm:text-2xl font-black text-white tracking-wide">
                      ...
                      {drillDownModalType === 'interns' && 'Student Interns Directory'}
                    </h3>
                    <span className="px-2.5 py-0.5 rounded-full bg-[#FFC000] text-[#002D62] text-xs font-black">
                      ...
                      {drillDownModalType === 'interns' && drillDownTraineesList.filter(t => t.roleCategory === 'intern').length}
                    </span>
                  </div>
                  <p className="text-xs text-blue-200 mt-0.5">
                    ...
                    {(drillDownModalType === 'trainees' || drillDownModalType === 'engineers' || drillDownModalType === 'technicians' || drillDownModalType === 'operators' || drillDownModalType === 'interns') && 'Individual trainee personnel records, departments, and course attendance history'}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-3">
              ...
              {/* VIEW 3: TRAINEES DIRECTORY (All, Engineers, Technicians, Operators, Interns) */}
              {(drillDownModalType === 'trainees' || drillDownModalType === 'engineers' || drillDownModalType === 'technicians' || drillDownModalType === 'operators' || drillDownModalType === 'interns') && (
                <div className="space-y-2.5">
                  {drillDownTraineesList
                    .filter(t => {
                      ...
                      if (drillDownModalType === 'interns') return t.roleCategory === 'intern';
                      return true;
                    })
                    .filter(t => {
